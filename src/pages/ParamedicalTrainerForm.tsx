...
const ParamedicalTrainerForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = `Name=${encodeURIComponent(formData.fullName)}&Email=${encodeURIComponent(formData.email)}&MobileNumber=${encodeURIComponent(formData.phone)}&Experience=${encodeURIComponent(formData.experience)}&Certifications=${encodeURIComponent(formData.certifications)}&TimeStamp=${encodeURIComponent(timestamp)}`;
      ...
      setSuccess(true);
      setFormData({ fullName: '', email: '', phone: '', experience: '', certifications: '' });
          ...
          <div className="form-card">
            ...
            {loading ? (
              ...
              <form className="form-container" onSubmit={handleSubmit}>
                ...
                <textarea className="form-input" name="experience" value={formData.experience} onChange={handleChange} required placeholder="Relevant Experience" rows={3}></textarea>
                <input className="form-input" type="text" name="certifications" value={formData.certifications} onChange={handleChange} required placeholder="Certifications" />
